/* ============================================================
   ...
   Renders the real guide photo from public/images/Guide01.jpg
   through Guide09.jpg.

   The guide list repeats the 9 base images across pages, so we
   map each card to the matching photo by modulo 9.
   ============================================================ */

import { useState } from "react";

export default function GuideAvatar({ avatar, name, guideId = 1 }) {
 const imageIndex = ((guideId - 1) % 9) + 1;
 const imageSrc = `/images/Guide${String(imageIndex).padStart(2, "0")}.jpg`;
 ...
 if (showFallback) {
   return (
     <svg viewBox="0 0 200 240" className="h-full w-full" role="img" aria-label={`Portrait of guide ${name}`}>
       <defs>
         <linearGradient id={`grad-${guideId}`} x1="0" y1="0" x2="1" y2="1">
           <stop offset="0%" stopColor={avatar.bg1} />
           <stop offset="100%" stopColor={avatar.bg2} />
         </linearGradient>
       </defs>
       <rect width="200" height="240" fill={`url(#grad-${guideId})`} />
       ...
       {avatar.hairStyle === "short" && (
         <path d="M60 100 C58 62 76 50 100 50 C124 50 142 62 140 100 C138 78 124 66 100 66 C76 66 62 78 60 100 Z" fill={avatar.hair} />
       )}
