...
import { PageStep, Guide, BookingState } from './types';
...
export default function App() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (json.data && Array.isArray(json.data) && json.data.length > 0) {
            setGuides(json.data);
            // Default selected guide is Narin
            const narin = json.data.find((g: Guide) => g.id === 'narin');
            if (narin) {
              setSelectedGuide(narin);
            }
          }
        }
      } catch (err) {
  ...
  const navigateToStep = (step: PageStep) => {
    setCurrentStep(step);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  // Triggered when user selects a guide on Page 1
