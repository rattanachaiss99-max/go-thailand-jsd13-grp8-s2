...
export function getOtherProperties(excludeId: string, count: number = 3) {
 ...
 */
export const facilityKeywords = Array.from(new Set(properties.flatMap((p) => p.keywords))).sort();
...
export const bedroomOptions = [
  { value: "1", label: "1 bedroom/studio", test: (bedrooms: number) => bedrooms === 1 },
  { value: "2", label: "2 bedrooms", test: (bedrooms: number) => bedrooms === 2 },
  { value: "3+", label: "3+ bedrooms", test: (bedrooms: number) => bedrooms >= 3 },
];
